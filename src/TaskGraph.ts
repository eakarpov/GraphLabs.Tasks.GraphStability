...
export class TaskGraph {
   ...
   public static getSubgraph(subVertices: Vertex[], graph: Graph<Vertex,Edge>): Graph<Vertex, Edge>{
     console.log("subGraph input", subVertices, graph);
      ...
      graph.edges.filter(e =>
              (subVertices.map(v => v.name).includes(<any>e.vertexOne as string))
              &&  (subVertices.map(v => v.name).includes(<any>e.vertexTwo as string))).forEach((e: Edge) => subGraph.addEdge(new Edge(e.vertexOne, e.vertexTwo)));
  ...
  public static getNeighbourhood(vertex: Vertex, graph: Graph<Vertex,Edge>): Vertex[] {
    /*return graph.vertices.reduce((accum: Vertex[], next: Vertex) => {
      ...
      graph.edges.forEach(e => {
        if (<any>e.vertexOne as string == vertex.name) {
          answerNames.push(<any>e.vertexTwo as string)
        }
        if (<any>e.vertexTwo as string == vertex.name) {
          answerNames.push(<any>e.vertexOne as string)
        }
      })
  ...
  public static getNonNeighbourhood(vertex: Vertex, graph: Graph<Vertex,Edge>): Vertex[] {
      ...
      let neighbours :Vertex[] = TaskGraph.getNeighbourhood(vertex, graph);
